...
Download the LTS version from nodejs.org. During installation, check "Automatically install necessary tools".
Verify installation:
powershell
Copy
...
Edit Path variable, add: %ANDROID_HOME%\platform-tools
Authentication
This app uses local authentication (no backend server). All user data is stored in AsyncStorage.
First-Time Setup
Open the app, tap "Sign Up"
Enter: Full Name (required), Phone (required), Password, Blood Type, Emergency Contact
Email is optional
Submit — you will be automatically logged in
Subsequent Logins
